...
func Salvar(res http.ResponseWriter, ID, token string) error {
...
func Ler(req *http.Request) (map[string]string, error) {
	cookie, erro := req.Cookie("dados")
	if erro != nil {
		return nil, erro
	}
...
// Deletar reseta os valores armazenados no cookie
func Deletar(res http.ResponseWriter) {
	http.SetCookie(res, &http.Cookie{
		Name:     "dados",
		Value:    "",
		Path:     "/",
